...
async fn announce(
    ...
    let info_hash = match query.get("info_hash") {
        Some(h) => h.clone(),
        None => return HttpResponse::BadRequest().body("Missing info_hash"),
    };
    ...
    let port = match query.get("port").and_then(|p| p.parse::<u16>().ok()) {
        Some(p) => p,
        None => return HttpResponse::BadRequest().body("Missing or invalid port"),
    };
    ...
    if event == "stopped" {
        ...
        match peers.iter_mut().find(|p| p.peer_id == peer_id) {
            Some(existing) => existing.last_seen = SystemTime::now(),
...
async fn main() -> std::io::Result<()> {
    ...
    println!("Tracker server running on http://0.0.0.0:8080");
